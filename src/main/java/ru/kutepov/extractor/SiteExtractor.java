...
public class SiteExtractor extends RecursiveAction {
    ...
    @Autowired
    public SiteExtractor(Set<String> urls, String pageUrl, SitesConfig sitesConfig) {
    ...
    protected void compute() {
        ...
        for (String childUrl : getChildUrls()) {
            if (!urls.contains(childUrl)) {                                      // Проверяем наличие ссылки в коллекции
                SiteExtractor task = new SiteExtractor(urls, childUrl, sitesConfig);          // В случае её отсутстви запускаем задачу для каждой дочерней ссылки
    ...
    public List<Integer> getSiteStatus() throws IOException {                           // Метод для получения кода ответа от запрашиваемой страницы
        ...
        for (int i = 0; i < urls.size(); i++) {
            Connection.Response response = Jsoup.connect(pageUrl).maxBodySize(0)
                    .userAgent(USER_AGENT)
                    .execute();
    ...
    public List<HashMap<String, String>> getField() throws IOException {
        ...
        for (String s : urls) {
            ...
            titleMap.put("title", elTitle.text());
            HashMap<String, String> bodyMap = new HashMap<>();
            bodyMap.put("body", elBody.text());
            list.add(titleMap);
            list.add(bodyMap);
        }
        return list;
